...
{
    final public const DATA_TYPE_STRING = 'ezpage';

    final public const DEFAULT_ZONE_LAYOUT_FIELD = 'data_text1';
        ...
        else
        {
            ...
            if ( $defaultLayout !== '' )
            {
                ...
                foreach ( $zones as $zoneIdentifier )
                {
                    ...
                    $newZone->setAttribute( 'id', md5( random_int(0, mt_getrandmax()) . microtime() . $page->getZoneCount() ) );
        ...
        switch ( $params[0] )
        {
            case 'new_zone_layout':
                if ( $http->hasPostVariable( 'ContentObjectAttribute_ezpage_zone_allowed_type_' . $contentObjectAttribute->attribute( 'id' ) ) )
                {
                    ...
                    $allowedZonesCount = is_countable($allowedZones) ? count( $allowedZones ) : 0;
            ...
            case 'set_rotation':
                ...
                if ( trim( (string) $rotationValue ) == '' || $rotationValue == 0 )
                {
                    $block->setAttribute( 'rotation', ['interval' => 0, 'type' => 0, 'value' => '', 'unit' => ''] );
                ...
                else
                {
                    ...
                    $block->setAttribute( 'rotation', ['interval' => $rotationInterval, 'type' => $rotationType, 'value' => $rotationValue, 'unit' => $rotationUnit] );
                }
                break;
            case 'remove_block':
                $page = $contentObjectAttribute->content();
            ...
            case 'new_block':
                ...
                $block->setAttribute( 'id', md5( random_int(0, mt_getrandmax()) . microtime() . $zone->getBlockCount() ) );
                $block->setAttribute( 'zone_id', $zone->attribute( 'id' ) );
                $block->setAttribute( 'type', $blockType );

                $blockState = 'id_' . $block->attribute( 'id' ) . '=1';
                setrawcookie( 'eZPageBlockState', isset( $_COOKIE['eZPageBlockState'] ) ? $_COOKIE['eZPageBlockState'] . '&' . $blockState : $blockState, ['expires' => time() + 3600, 'path' => '/'] );
                break;
            case 'move_block_up':
                $page = $contentObjectAttribute->content();
                $zone = $page->getZone( $params[1] );
            ...
            case 'new_item_browse':
                ...
                $browseParameters = ['class_array' => $classArray, 'action_name' => 'AddNewBlockItem', 'browse_custom_action' => ['name' => 'CustomActionButton[' . $contentObjectAttribute->attribute( 'id' ) . '_new_item-' . $params[1] . '-' . $params[2] . ']', 'value' => $contentObjectAttribute->attribute( 'id' )], 'from_page' => $redirectionURI, 'cancel_page' => $redirectionURI, 'persistent_data' => ['HasObjectInput' => 0], 'start_node' => $this->getBrowseStartNodeId( $http, $blockINI, $contentObjectAttribute, $block, $params )];

                eZContentBrowse::browse( $module, $browseParameters );
                break;
            case 'new_source':
                $page = $contentObjectAttribute->content();
            ...
            case 'new_source_browse':
                ...
                $browseParameters = ['action_name' => 'AddNewBlockSource', 'selection' => $fetchParametersSelectionType['Source'], 'browse_custom_action' => ['name' => 'CustomActionButton[' . $contentObjectAttribute->attribute( 'id' ) . '_new_source-' . $params[1] . '-' . $params[2] . ']', 'value' => $contentObjectAttribute->attribute( 'id' )], 'from_page' => $redirectionURI, 'cancel_page' => $redirectionURI, 'persistent_data' => ['HasObjectInput' => 0]];
            ...
            case 'custom_attribute_browse':
                ...
                $browseParameters = ['action_name' => 'CustomAttributeBrowse', 'browse_custom_action' => ['name' => 'CustomActionButton[' . $contentObjectAttribute->attribute( 'id' ) . '_custom_attribute-' . $params[1] . '-' . $params[2] . '-' . $params[3] . ']', 'value' => $contentObjectAttribute->attribute( 'id' )], 'from_page' => $redirectionURI, 'cancel_page' => $redirectionURI, 'persistent_data' => ['HasObjectInput' => 0]];
                            ...
                            if ( $block->getItemCount() != 0 )
                            {
                                foreach ( $block->attribute( 'items' ) as $item )
                                {
                                    switch ( $item->attribute( 'action' ) )
                                    {
                                        ...
                                        case 'add':
                                            $newItems[] =  ['blockID' => $blockID, 'objectID' => $item->attribute( 'object_id' ), 'nodeID' => $item->attribute( 'node_id' ), 'priority' => $item->attribute( 'priority' ), 'timestamp' => $item->attribute( 'ts_publication' )];
                                            break;
