...
class eZPageSwapType extends eZWorkflowEventType
{
    ...
    final public const WORKFLOW_TYPE_STRING = 'ezpageswap';
    ...
    function __construct()
    {
        ...
        $this->setTriggerTypes( ['content' => ['swap' => ['after']]] );
    }
    ...
    function execute( $process, $event )
    {
        ...
        $oldNodeBlocks = eZFlowBlock::fetchObjectList( eZFlowBlock::definition(), null, ['node_id' => $oldNodeID] );
        $newNodeBlocks = eZFlowBlock::fetchObjectList( eZFlowBlock::definition(), null, ['node_id' => $newNodeID] );
        ...
        $swappedBlockIDs = [];
        $oldNodePoolItems = eZFlowPoolItem::fetchObjectList( eZFlowPoolItem::definition(), null, ['node_id' => $oldNodeID] );
        $newNodePoolItems = eZFlowPoolItem::fetchObjectList( eZFlowPoolItem::definition(), null, ['node_id' => $newNodeID] );
        ...
        foreach( $oldNodePoolItems as $oldNodePoolItem )
        {
            ...
            $swappedNodePoolItem = eZFlowPoolItem::fetchObjectList( eZFlowPoolItem::definition(), null, ['node_id' => $newNodeID, 'block_id' => $oldNodePoolItem->attribute( 'block_id' )] );
            if( empty( $swappedNodePoolItem ) )
            {
                $updateParameters = ['definition' => eZFlowPoolItem::definition(), 'update_fields' => ['object_id' => $oldNodeObjectID], 'conditions' => ['object_id' => $oldNodePoolItem->attribute( 'object_id' ), 'block_id' => $oldNodePoolItem->attribute( 'block_id' )]];
        ...
        foreach( $newNodePoolItems as $newNodePoolItem )
        {
            ...
            $updateParameters = ['definition' => eZFlowPoolItem::definition(), 'update_fields' => ['object_id' => $newNodeObjectID], 'conditions' => ['object_id' => $newNodePoolItem->attribute( 'object_id' ), 'block_id' => $newNodePoolItem->attribute( 'block_id' )]];
            eZFlowPoolItem::updateObjectList( $updateParameters );
        }
