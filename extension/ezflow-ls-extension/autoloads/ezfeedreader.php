...
class eZFeedReader
{
    ...
    {
        return ['feedreader' => ['source' => ['type' => 'string', 'required' => true, 'default' => ''], 'limit' => ['type' => 'integer', 'required' => false, 'default' => 0], 'offset' => ['type' => 'integer', 'required' => false, 'default' => 0]]];
    }
     ...
     */
    public function modify( $tpl, $operatorName, $operatorParameters, $rootNamespace, $currentNamespace, mixed &$operatorValue, $namedParameters )
    {
        switch ( $operatorName )
        {
            case 'feedreader':
                $source = $namedParameters['source'] ?? '';
                $limit = $namedParameters['limit'] ?? 0;
                $offset = $namedParameters['offset'] ?? 0;
                $res = [];
                ...
                $res['links'] = self::buildLinksArray( $feed->link ?? [] );
                ...
                foreach( $items as $item )
                {
                    ...
                    $links = self::buildLinksArray( $item->link ?? [] );

                    $res['items'][] = ['title' => $title, 'links' => $links, 'description' => $description, 'content' => $content, 'published' => $published];
