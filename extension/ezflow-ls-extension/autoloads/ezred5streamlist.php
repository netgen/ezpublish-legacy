...
class eZRed5StreamListOperator
{
    ...
    function namedParameterList()
    {
        ...
        return ['red5list' => ['fileserver' => ['type' => 'string', 'required' => true, 'default' => ""], 'key' => ['type' => 'string', 'required' => true, 'default' => ""]]];
    }
    ...
    function buildURL( $urlPartsArray )
    {
        ...
        foreach( $urlPartsArray as $urlPart )
        {
            ...
            // Detect trailing /
            if ( ( strrpos( (string) $url, "/" ) + 1 ) == strlen( (string) $url ) )
            {
                $url .= $urlPart;
            }
            else
    ...
    */
   function modify( $tpl, $operatorName, $operatorParameters, $rootNamespace, $currentNamespace, mixed &$operatorValue, $namedParameters )
    {
        ...
        $remoteHandle = fopen( $this->buildURL( [$red5ListURL, "flash_video_list.php", '?key=' . $key] ), "r" );
        if ( !$remoteHandle )
            return "";
        ...
        foreach( $fileArray as $file )
        {
            ...
            $absoluteURL = $this->buildURL( [$red5ListURL, $file] );
            $returnArray[$name] = ["filename" => $file, "absoluteURL" => $absoluteURL];
        }
