...
foreach ( $zones as $zone )
{
    ...
    foreach ( $blocks as $block )
    {
        ...
        $outputBlock = ['objectid' => $block->attribute('zone_id') . '-' . $block->attribute('id')];
        $tpl->setVariable( 'block', $block );
        $outputBlock['xhtml'] = htmlentities( str_replace( ["\r\n", "\r", "\n"], [""], (string) $tpl->fetch( 'design:page/preview.tpl' ) ), ENT_QUOTES, $httpCharset );
        $output[] = $outputBlock;
    }
}
