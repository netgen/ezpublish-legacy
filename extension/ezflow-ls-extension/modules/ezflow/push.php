...
if ( $Module->isCurrentAction( 'Store' ) && $Module->hasActionParameter( 'PlacementList' ) )
{
    ...
    foreach ( $Module->actionParameter( 'PlacementList' ) as $zones )
    {
        foreach ( $zones as $blocks)
        {
            ...
            {
                $newItems[] = ['blockID' => $blockID, 'objectID' => $object->attribute( 'id' ), 'nodeID' => $node->attribute( 'node_id' ), 'priority' => 0, 'timestamp' => $timestamp];
            }
        }
    }
...
$Result['path'] = [['url' => false, 'text' => ezpI18n::tr( 'ezflow/push', 'Push to block' )]];;
