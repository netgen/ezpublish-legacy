...
$validCount = is_countable($block->attribute('valid')) ? count( $block->attribute('valid') ) : 0;
$waitingCount = is_countable($block->attribute('waiting')) ? count( $block->attribute('waiting') ) : 0;
...
function createParamsFromStr( $str )
{
    ...
    foreach( explode( '_', (string) $str ) as $id )
    {
        $elem = explode( ':', $id );
