...
switch ( strtolower( (string) $output ) )
{
    case 'json':
        ...
        {
            if ( !in_array( $attr, ['waiting', 'valid', 'valid_nodes', 'archived'] ) )
                $obj->$attr = $block->attribute($attr);
        }
        $obj->html = htmlentities((string) $tpl->fetch($template), ENT_QUOTES);

        header('Content-type: application/json');
        echo json_encode(['block' => $obj]);
        break;
    case 'xml':
