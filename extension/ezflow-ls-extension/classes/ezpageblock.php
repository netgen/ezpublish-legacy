...
class eZPageBlock
{
    ...
    private array $dynamicAttributeFunctions = ['waiting' => 'getWaitingItems', 'valid' => 'getValidItems', 'valid_nodes' => 'getValidItemsAsNodes', 'archived' => 'getArchivedItems', 'view_template' => 'viewTemplate', 'edit_template' => 'editTemplate', 'last_valid_item' => 'getLastValidItem'];
    ...
    {
        return isset( $this->attributes['items'] ) ? is_countable($this->attributes['items']) ? count( $this->attributes['items'] ) : 0 : 0;
    }
    ...
    {
        return [...array_keys( $this->attributes ), ...array_keys( $this->dynamicAttributeFunctions )];
    }
    ...
    public function attribute( $name )
    {
        if ( isset( $this->dynamicAttributeFunctions[$name] ) )
        {
            ...
            {
                $attribute = call_user_func( [$this, $this->dynamicAttributeFunctions[$name]] );
                return $attribute;
            }
        }
        else
    ...
    static public function fetch( $blockID, $asObject = true )
    {
        ...
        if ( $pageBlock !== null )
        {
            ...
            $row[0] += ["id" => $blockID, "view" => $pageBlock->attribute( "view" )];
        }
    ...
    protected function getValidItems()
    {
        ...
        usort( $merged, $this->sortItemsByPriority(...) );
    ...
    public function __clone()
    {
        ...
        $oldItems = eZPersistentObject::fetchObjectList( eZFlowPoolItem::definition(), null, ['block_id' => $oldBlockID] );

        foreach ( $oldItems as $oldItem )
        {
            $attrs = ['object_id' => $oldItem->attribute( 'object_id' ), 'node_id' => $oldItem->attribute( 'node_id' ), 'priority' => $oldItem->attribute( 'priority' ), 'ts_publication' => $oldItem->attribute( 'ts_publication' ), 'action' => 'add'];
