...
class eZFlowAjaxContent
{
        ...
        if ( is_array( $params['dataMap'] ) && is_array(  $params['dataMapType'] ) )
        {
            ...
            foreach( $dataMap as $key => $atr )
            {
                ...
                if ( in_array( $dataTypeString, $params['imageDataTypes'], true) !== false )
                {
                    ...
                    $imageArray['original']             = ['url' => $attrtibuteArray[ $key ]['content']];
     ...
     * @return string
     */
    public static function jsonEncode( mixed $obj )
    {
     ...
     * @return string
     */
    protected function phpJsonEncode( mixed $var )
    {       
        switch (gettype($var)) {
            case 'boolean':
            ...
            case 'array':
                ...
                */
                ...
                if (is_array($var) && count($var) && (array_keys($var) !== range(0, sizeof($var) - 1))) {
                    return sprintf('{%s}', join(',', array_map($this->phpJsonEncodeNameValue(...),
                                                               array_keys($var),
                                                               array_values($var))));
                }

                // treat it like a regular array
                return sprintf('[%s]', join(',', array_map($this->phpJsonEncode(...), $var)));
   ...
   /**
     ...
     * @return string
     */
    protected function phpJsonEncodeNameValue( $name, mixed $value )
    {
