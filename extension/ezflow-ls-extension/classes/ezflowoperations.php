...
{
    final public const ROTATION_NONE = 0;
    final public const ROTATION_SIMPLE = 1;
    final public const ROTATION_RANDOM = 2;
    ...
    public static function updateBlockPoolByBlockID( $block, $publishedBeforeOrAt = false )
    {
        ...
        {
            $newItems[] = ['blockID' => $block['id'], 'objectID' => $item['object_id'], 'nodeID' => $item['node_id'], 'priority' => 0, 'timestamp' => $item['ts_publication']];
        }
     ...
     * @static
     */
    public static function update( $nodeArray = [] )
    {
        ...
        foreach ( $nodeArray as $nodeID )
        {
            ...
            {
                $next[$block['id']] = trim( (string) $block['overflow_id'] ); // Make sure that block ID does not any have spaces
                ...
                if ( $movingFromQueue )
                {
                    ...
                    if ( $countToRemove > 0 )
                    {
                        ...
                        $items = $db->arrayQuery( "SELECT node_id, object_id, rotation_until
                                           ...
                                           WHERE block_id='$blockID'
                                             AND ts_visible>0
                                             AND ts_hidden=0
                                           ORDER BY priority ASC", ['limit' => $countToRemove] );
                            ...
                            foreach( $items as $item )
                            {
                                ...
                                $item['rotation_until'] == 0 ) )
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        $newPublicationTS = 0;
                                        $priority = random_int(0, mt_getrandmax());
                                    }
                                    ...
                                    foreach( $items as $item )
                                    {
                                        ...
                                        $duplicityCheck = $db->arrayQuery( "SELECT object_id
                                                                    ...
                                                                    WHERE block_id='$overflowID'
                                                                      AND object_id=$itemObjectID", ['limit' => 1] );
                                        if ( $duplicityCheck )
                                        {
                    ...
                    if ( $countToRemove > 0 )
                    {
                        $items = $db->arrayQuery( "SELECT object_id
                                           ...
                                           WHERE block_id='$blockID'
                                             AND ts_hidden>0
                                           ORDER BY ts_hidden ASC", ['limit' => $countToRemove] );
    ...
    public static function cleanupRemovedItems()
    {
        ...
        do
        {
            $items = $db->arrayQuery( 'SELECT node_id FROM ezm_pool', ['offset' => $offset, 'limit' => $limit] );
            if ( empty( $items ) )
                break;
