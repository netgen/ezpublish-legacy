...
class eZFlowLatestObjects implements eZFlowFetchInterface
{
    public function fetch( $parameters, $publishedAfter, $publishedBeforeOrAt )
    {
        ...
        $subTreeParameters['AttributeFilter'] = ['and', ['published', '>', $publishedAfter], ['published', '<=', $publishedBeforeOrAt], ['visibility', '=', true]];

        if ( isset( $parameters['Class'] ) )
        {
            $subTreeParameters['ClassFilterType'] = 'include';
            $subTreeParameters['ClassFilterArray'] = explode( ';', (string) $parameters['Class'] );
        }
        ...
        {
            $fetchResult[] = ['object_id' => $item['id'], 'node_id' => $item['node_id'], 'ts_publication' => $item['published']];
        }
