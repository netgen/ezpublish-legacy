...
class eZFlowKeywordsFetch implements eZFlowFetchInterface
{
    public function fetch( $parameters, $publishedAfter, $publishedBeforeOrAt )
    {
        ...
        {
            $classIdentifiers = explode( ',', (string) $parameters['Classes'] );
        ...
        $result = eZFunctionHandler::execute( 'content','keyword', ['alphabet' => $keywords, 'classid' => $classIDs, 'offset' => 0, 'limit' => $limit, 'parent_node_id' => $nodeID, 'include_duplicates' => false, 'sort_by' => $sortBy, 'strict_matching' => false] );
        ...
        {
            $fetchResult[] = ['object_id' => $item['link_object']->attribute( 'contentobject_id' ), 'node_id' => $item['link_object']->attribute( 'node_id' ), 'ts_publication' => $item['link_object']->object()->attribute( 'published' )];
        }
