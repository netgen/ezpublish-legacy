...
class eZPage
{
    ...
    public static function createFromXML( $source )
    {
        ...
        if ( $zoneINI->hasVariable( $layoutName, 'Zones' ) )
        {
            foreach ( $zoneINI->variable( $layoutName, 'Zones' ) as $zoneIdentifier )
            {
                ...
                $newZone->setAttribute( 'id', md5( random_int(0, mt_getrandmax()) . microtime() . $newObj->getZoneCount() ) );
    ...
    {
        return isset( $this->attributes['zones'] ) ? is_countable($this->attributes['zones']) ? count( $this->attributes['zones'] ) : 0 : 0;
    }
