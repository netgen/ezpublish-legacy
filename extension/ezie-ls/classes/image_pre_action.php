 ...
 * @package ezie
 */
class eZIEImagePreAction implements \Stringable
{
    ...
    private function prepare_region()
    {
        ...
        if ( $http->hasVariable( 'selection' ) )
        {
            ...
            {
                $region = ['x' => intval( $selection['x'] ), 'y' => intval( $selection['y'] ), 'w' => intval( $selection['w'] ), 'h' => intval( $selection['h'] )];
            }
        }
    ...
    public function __toString(): string
    {
        ...
        return json_encode( $stringObject, JSON_THROW_ON_ERROR );
    }
