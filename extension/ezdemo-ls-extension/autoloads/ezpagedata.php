...
class eZPageData
{
    ...
    {
        return ['ezpagedata', 'ezpagedata_set', 'ezpagedata_append'];
    }
    ...
    {
        return ['ezpagedata' => ['params' => ['type' => 'array', 'required' => false, 'default' => []]], 'ezpagedata_set' => ['key' => ['type' => 'string', 'required' => true, 'default' => false], 'value' => ['type' => 'mixed', 'required' => true, 'default' => false]], 'ezpagedata_append' => ['key' => ['type' => 'string', 'required' => true, 'default' => false], 'value' => ['type' => 'mixed', 'required' => true, 'default' => false]]];
    }
    ...
    function modify( $tpl, $operatorName, $operatorParameters, $rootNamespace, $currentNamespace, &$operatorValue, $namedParameters )
    {
        switch ( $operatorName )
        {
            ...
            case 'ezpagedata':
            {
                ...
                else if ( isset( $moduleResult['path'][(is_countable($moduleResult['path']) ? count( $moduleResult['path'] ) : 0) - 1]['node_id'] ) )
                {
                   $currentNodeId = (int) $moduleResult['path'][(is_countable($moduleResult['path']) ? count( $moduleResult['path'] ) : 0) - 1]['node_id'];
                }
                ...
                $pageData['main_node_id'] = $contentInfo['main_node_id'] ?? $currentNodeId;
                ...
                $pageData['page_depth']          = is_countable($moduleResult['path']) ? count( $moduleResult['path'] ) : 0;
                ...
                if ( $uiContext === 'edit'
                  && !str_contains( (string) $uriString, 'user/edit' )
                  && ( empty( $contentInfo ) || !str_contains( (string) $uriString, 'content/action' ) ) )
                {
                      ...
                      && $contentInfo['current_language'] !== $ini->variable( 'RegionalSettings', 'ContentObjectLocale' ) )
                {
                    ...
                    if ( $siteaccess !== null )
                    {
                        ...
                        if ( ( $contentInfo['language_mask'] & $lang->attribute('id') ) < 1 )
                        {
                            ...
                            $handlerOptions->handlerParams = [['Parameters' => ['sa', $currentNodeId], 'UserParameters' => []]];
                ...
                */
                ...
                else if ( $viewMode === 'sitemap' || $viewMode === 'tagcloud' || str_starts_with((string) $uriString, 'content/versionview') )
                {
                    $pageData['website_toolbar'] = false;
                }
                ...
                $pageData['extra_menu_class_list'] = ['infobox'];
                ...
                if ( $menuIni->variable( 'MenuSettings', 'AlwaysAvailable' ) === 'false' )
                {
                    ...
                    if ( $pageData['is_edit'] || str_starts_with((string) $uriString, 'content/versionview') )
                    {
                ...
                else if ( $pageData['extra_menu'] && $pageData['extra_menu_class_list'] && $pageData['extra_menu_node_id'] )
                {
                    ...
                    {
                        $pageData['extra_menu_subitems'] = eZContentObjectTreeNode::subTreeCountByNodeID( ['Depth' => 1, 'DepthOperator'    => 'eq', 'ClassFilterType'  => 'include', 'ClassFilterArray' => $pageData['extra_menu_class_list']],
                ...
                $path = ( isset( $moduleResult['path'] ) && is_array( $moduleResult['path'] ) ) ? $moduleResult['path'] : [];
    ...
    static public function getNodePath( $nodeId )
    {
        ...
        {
            $path['path'][] = ['text' => $parent->attribute( 'name' ), 'url' => '/content/view/full/' . $parent->attribute( 'node_id' ), 'url_alias' => $parent->attribute( 'url_alias' ), 'node_id' => $parent->attribute( 'node_id' )];
        }

        $path['title_path'] = $path['path'];
        $path['path'][] = ['text' => $node->attribute( 'name' ), 'url' => false, 'url_alias' => false, 'node_id' => $node->attribute( 'node_id' )];

        $path['title_path'][] = ['text' => $node->attribute( 'name' ), 'url' => false, 'url_alias' => false];
        return $path;
    }
