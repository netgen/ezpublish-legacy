...
class ezpRestAtomController extends ezcMvcController
{
    ...
    public function doCollection()
    {
        ...
        {
            $retData[] = ["objectName" => $node->name, "author" => $node->owner->Name, "modified" => $node->dateModified, "published" => $node->datePublished, "classIdentifier" => $node->classIdentifier, "nodeUrl" => $baseUri . $this->getRouter()->generateUrl( 1, ["nodeId" => $node->locations->node_id] )];
        }
