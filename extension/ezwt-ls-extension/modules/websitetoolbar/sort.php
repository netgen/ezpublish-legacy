...
$Result = [];
$node   = false;
$viewParameters = ['offset' => $Params['Offset'], 'year' => $Params['Year'], 'month' => $Params['Month'], 'day' => $Params['Day'], 'namefilter' => false];
...
{
    $Result['content'] = ezpI18n::tr( 'design/standard/websitetoolbar/sort', 'Invalid or missing parameter: %parameter', null, ['%parameter' => 'NodeID'] );
    return $Result;
}
...
$titlePath = $path;
$path[] = ['text' => $node->attribute( 'name' ), 'url' => false, 'url_alias' => false, 'node_id' => $node->attribute( 'node_id' )];

$titlePath[] = ['text' => $node->attribute( 'name' ), 'url' => false, 'url_alias' => false];
