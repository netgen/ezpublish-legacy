...
{
    final public const DATA_TYPE_STRING = 'ezgmaplocation';
    ...
    function fetchObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        if ( $validPostData )
        {
        	...
        	if ( $http->hasPostVariable( $base . '_data_gmaplocation_address_' . $contentObjectAttribute->attribute( 'id' ) ) )
            {
                ...
                $address = htmlentities( (string) $address, ENT_QUOTES , 'UTF-8' );
            }
            ...
            {
		        $location = new eZGmapLocation( ['contentobject_attribute_id' => $contentObjectAttribute->attribute('id'), 'contentobject_version' => $contentObjectAttribute->attribute('version'), 'latitude' => $latitude, 'longitude' => $longitude, 'address' => $address] );
    ...
    {
    	$data = $string !== '' && str_contains( $string, '|#' ) ? explode( '|#', $string ) : [0];

    	if ( $data[0] != 0 )
    	{
            $location = new eZGmapLocation( ['contentobject_attribute_id' => $contentObjectAttribute->attribute('id'), 'contentobject_version' => $contentObjectAttribute->attribute('version'), 'latitude' => $data[1], 'longitude' => $data[2], 'address' => $data[3]]);
            $contentObjectAttribute->setContent( $location );
    	}
    ...
    function unserializeContentObjectAttribute( $package, $contentObjectAttribute, $attributeNode )
    {
        ...
        {
            $location = new eZGmapLocation( ['contentobject_attribute_id' => $contentObjectAttribute->attribute('id'), 'contentobject_version' => $contentObjectAttribute->attribute('version'), 'latitude' => $attributeNode->getElementsByTagName( 'latitude' )->item( 0 )->textContent, 'longitude' => $attributeNode->getElementsByTagName( 'longitude' )->item( 0 )->textContent, 'address' => $attributeNode->getElementsByTagName( 'address' )->item( 0 )->textContent]);
            $contentObjectAttribute->setContent( $location );
        }
    }
}
