...
if ( $http->hasPostVariable( "ExportButton" ) )
{
    eZContentBrowse::browse( $module,
                             ['action_name' => 'OOPlace', 'description_template' => 'design:ezodf/browse_place.tpl', 'content' => [], 'from_page' => '/ezodf/export/', 'cancel_page' => '/ezodf/export/'] );
    return;
}
...
else if ( strlen( trim( (string) $exportTypeParam ) ) != 0 )
{
    $tpl->setVariable( "error_string", ezpI18n::tr( 'extension/ezodf/export/error', "Destination file format not supported" ) );
    $success = false;
}
...
if ( $doExport == true )
{
    if ( is_numeric( $nodeID ) )
    {
        ...
        if ( $success )
        {
            ...
            if ( $contentLength > 0 )
            {
                $escapedOriginalFileName = addslashes( (string) $originalFileName );
            ...
            {
                $tpl->setVariable( "error_string", ezpI18n::tr( 'extension/ezodf/export/error', "Unable to open file %1 on server side", null, [$fileName] ) );
            }
        }
    }
}
...
$Result['path'] = [['url' => '/ezodf/export/', 'text' => ezpI18n::tr( 'extension/ezodf', 'OpenOffice.org export' )]];
