...
//
    ...
    else
    {
        ...
        $nodes = eZFunctionHandler::execute( 'content','list', ['parent_node_id' => $parentNodeID] );
        ...
        foreach( $nodes as $node )
        {
            ...
            preg_match( '/\/+[a-z0-9\-\._]+\/?[a-z0-9_\.\-\?\+\/~=&#;,]*[a-z0-9\/]{1}/si', (string) $tpl->fetch( 'design:ezodf/icon.tpl' ), $matches );
            $iconPath = 'http://'. eZSys::hostname(). ':' . eZSys::serverPort() . $matches[0];
            $array[] = [$nodeID, $name, $className, $isContainer, $iconPath];
        }
