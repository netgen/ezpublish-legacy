...
class eZOOConverter
{
    ...
    */
    static function objectToOO( $nodeID )
    {
        ...
        $supportedDatatypes = ['ezstring', 'eztext', 'ezxmltext', 'ezimage', 'ezdate', 'ezdatetime', 'ezmatrix'];
        ...
        $enabledClassAttributes = $odfINI->hasVariable( $classIdentifier, 'Attribute' ) ? $odfINI->variable( $classIdentifier, 'Attribute' ) : [];
        ...
        foreach ( $attributes as $attribute )
        {
            ...
            switch ( $datatype )
            {
                case "ezstring":
                {
                    $text = trim( (string) $attribute->content() );
                    if ( $text != "" )
                    {
                ...
                case "ezimage":
                {
                    ...
                    $imageArray = ["FileName" => $uniqueFile, "Alignment" => "center", "DisplayWidth" => $originalImage['width'], "DisplayHeight" => $originalImage['height']];
    ...
    static function handleNode( $node, $generator, $level = 0 )
    {
        switch ( $node->localName )
        {
            ...
            case "header":
            {
                ...
                $generator->addHeader( trim( (string) $node->nodeValue ), $level, $paragraphParameters );
            } break;
            ...
            case "paragraph":
            {
                ...
                call_user_func_array( [$generator, "addParagraph"], $paragraphParameters );
            } break;
    ...
    static function handleInlineNode( $child, $generator, $prevLineBreak = false )
    {
        ...
        switch ( $child->localName )
        {
            case "line":
            {
                ...
                if ( $prevLineBreak )
                {
                    $paragraphParameters[] = [eZOOGenerator::LINE, ''];
                }

                // Todo: support inline tags
                $paragraphParameters[] = [eZOOGenerator::TEXT, $child->textContent];
                    ...
                    if ( $lineChild->localName == 'embed' )
                    {
                        ...
                        if ( $object && $object->canRead() )
                        {
                            ...
                            if ( $classIdentifier == "image" )
                            {
                                ...
                                $imageArray[] = ["FileName" => $uniqueFile, "Alignment" => $imageAlignment, "DisplayWidth" => $displayWidth, "DisplayHeight" => $displayHeight];
                            }
                        }
                        else
                        {
            ...
            case "link":
            {
                ...
                $paragraphParameters[] = [eZOOGenerator::LINK, $href, $child->textContent];
            } break;
            ...
            case "strong":
            {
                ...
                $paragraphParameters[] = [eZOOGenerator::STYLE_START, $style];
                ...
                $paragraphParameters[] = [eZOOGenerator::STYLE_STOP];
            } break;
            ...
            case "custom":
            {
                ...
                if ( $isInline )
                {
                    $paragraphParameters[] = [eZOOGenerator::STYLE_START, "eZCustominline_20_$customTagName"];
                    $paragraphParameters[] = [eZOOGenerator::TEXT, $child->textContent];
                    $paragraphParameters[] = [eZOOGenerator::STYLE_STOP];
                }
                else
        ...
        return ["paragraph_parameters" => $paragraphParameters, "image_array" =>  $imageArray];
    }
}
