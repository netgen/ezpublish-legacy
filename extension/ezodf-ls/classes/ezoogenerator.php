...
class eZOOGenerator
{
    final public const TEXT = 1001;
    final public const LINK = 1002;
    final public const LINE = 1003;
    final public const STYLE_START = 2003;
    final public const STYLE_STOP = 2004;

    final public const ERROR_TEMPLATE_NOT_READABLE = 1010;
    final public const ERROR_COULD_NOT_COPY = 1011;
    ...
    */
    ...
    function writeDocument( )
    {
        ...
        if ( $useTemplate )
        {
            ...
            {
                $templateRepository = trim( (string) $ooINI->variable( 'ODFExport', 'TemplateRepository' ), '/' );   
            }
            ...
            {
                return [self::ERROR_COULD_NOT_COPY, "Could not copy the styles.xml file."];
            }
    ...
    */
    function addHeader( $text, $level = 1, $paragraphArray = [] )
    {
        $headerContents = [];
        ...
        foreach ( $paragraphArray as $paragraphElement )
        {
            switch ( $paragraphElement[0] )
            {
                case self::TEXT:
                {
                    ...
                    $tagContent = str_replace( "&", "&amp;", (string) $tagContent );
                    ...
                    $headerContents['Element'][] = ['Type' => 'text', "Content" => $tagContent];
                }break;

                case self::LINK:
                {
                    $headerContents['Element'][] = ['Type' => 'link', "Content" => $content = $paragraphElement[2], "HREF" => $paragraphElement[1]];
                }break;

                case self::LINE:
                {
                    $headerContents['Element'][] = ['Type' => 'line', "Content" => ''];
                }break;
        ...
        $this->DocumentArray[] = ['Type' => 'header', 'Content' => $headerContents, 'Level' => $level];
    }
    ...
    */
    function addParagraph()
    {
            ...
            foreach ( $argArray as $paragraphElement )
            {
                switch ( $paragraphElement[0] )
                {
                    case self::TEXT:
                    {
                        ...
                        $paragraphArray[] = ['Type' => 'text', "Content" => $tagContent];
                    } break;

                    case self::STYLE_START:
                    {
                        if ( $paragraphElement[1] == "bold" )
                            $paragraphArray[] = ['Type' => 'bold_start'];
                        if ( $paragraphElement[1] == "italic" )
                            $paragraphArray[] = ['Type' => 'italic_start'];

                        if ( str_starts_with((string) $paragraphElement[1], "eZCustominline_20_") )
                            $paragraphArray[] = ['Type' => 'custom_inline_start', 'Name' => $paragraphElement[1]];

                    } break;

                    case self::STYLE_STOP:
                    {
                        $paragraphArray[] = ['Type' => 'style_stop'];
                    } break;

                    case self::LINK:
                    {
                        $paragraphArray[] = ['Type' => 'link', "Content" => $content = $paragraphElement[2], "HREF" => $paragraphElement[1]];
                    } break;

                    case self::LINE:
                    {
                        $paragraphArray[] = ['Type' => 'line', "Content" => ''];
                    }break;
        ...
        else if ( $numArgs > 0 )
        {
            ...
            $paragraphArray = [['Type' => 'text', "Content" => $argArray[0] ?? '']];
        }
        else
        {
            $paragraphArray = [['Type' => 'text', "Content" => '']];
        }

        $elementArray = ['Type' => 'paragraph', 'Style' => $style, 'Content' => $paragraphArray];
        ...
        else
        {
            ...
            else
            {
                ...
                if ( isset( $this->DocumentStack[$this->CurrentStackNumber]['CurrentColSpan'] )
                    ...
                    $elementArray = array_merge( $elementArray, ["ColSpan" => $this->DocumentStack[$this->CurrentStackNumber]['CurrentColSpan']] );
    ...
    */
    ...
    {
        $elementArray = ['Type' => 'image', 'SRC' => $fileName['FileName'], 'Alignment' => $fileName['Alignment'], 'DisplayWidth' => $fileName['DisplayWidth'], 'DisplayHeight' => $fileName['DisplayHeight']];
    ...
    */
    function startList( $type = "unordered" )
    {
        ...
        $this->DocumentStack[$this->CurrentStackNumber]['ChildArray'] = [];
    }
    ...
    */
    function endList()
    {
        ...
        {
            $listItemArray[] = ['Type' => 'listitem', 'Content' => $listItem];
        }
        ...
        if ( $this->CurrentStackNumber == 0 )
        {
            $this->DocumentArray[] = ['Type' => 'list', 'ListType' => $this->DocumentStack[$this->CurrentStackNumber + 1]['ListType'], 'Content' => $listItemArray];
        }
        else
        {
            $elementArray = ['Type' => 'list', 'ListType' => $this->DocumentStack[$this->CurrentStackNumber + 1]['ListType'], 'Content' => $listItemArray];
    ...
    */
    ...
    {
        $this->DocumentArray[] = ['Type' => 'class-map-header', 'Text' => $name];
    }
    ...
    */
    ...
    {
        $this->DocumentArray[] = ['Type' => 'section', 'Text' => $name];
    }
    ...
    */
    function endSection( )
    {
        $this->DocumentArray[] = ['Type' => 'end-section'];
    }
    ...
    */
    function endTable()
    {
        ...
        if ( $this->CurrentStackNumber == 0 )
        {
            $this->DocumentArray[] = ['Type' => 'table', 'Content' => $this->DocumentStack[1]['ChildArray']];
        }
    }
    ...
    function handleElement( $element )
    {
        ...
        switch ( $element['Type'] )
        {
            case "paragraph":
            {
                ...
                {
                    match ($paragraphElement['Type']) {
                        "text" => $contentXML .=  $paragraphElement['Content'],
                        "custom_inline_start" => $contentXML .=  "<text:span text:style-name='" . $paragraphElement['Name'] . "'>",
                        "bold_start" => $contentXML .=  "<text:span text:style-name='T1'>",
                        "italic_start" => $contentXML .=  "<text:span text:style-name='T2'>",
                        "style_stop" => $contentXML .=  "</text:span>",
                        "link" => $contentXML .= "<text:a xlink:type='simple' xlink:href='" . $paragraphElement['HREF']. "'>" . $paragraphElement['Content'] . "</text:a>",
                        "line" => $contentXML .= "<text:line-break />",
                        default => eZDebug::writeError( "Unsupported paragraph element" ),
                    };
                }
                $contentXML .= "</text:p>";
                    ...
                    foreach( $element['Content']['Element'] as $headerElement )
                    {
                        switch ( $headerElement['Type'] )
                        {
                            ...
                            case "link":
                            {
                                ...
                                if( strlen( (string) $element['ClassName'] ) )
                                {
            ...
            case "image" :
            {
                $uniquePart = substr( md5( time() . random_int( 0, 20000 ) ), 6 );
                $fileName = $element['SRC'];
                $relativeFile = "Pictures/" . $uniquePart . basename( (string) $fileName );
    ...
    public $IsInsideTableHeading = false;
    public $CurrentStackNumber = 0;
    public $DocumentStack = [];
    public $DocumentArray = [];

    public $ImageFileArray = [];
    public $SourceImageArray = [];

    public $OORootDir = "var/cache/ezodf/";
    public $OOExportDir = "var/cache/ezodf/export/";
    public $OOTemplateDir = "var/cache/ezodf/template/";
}
