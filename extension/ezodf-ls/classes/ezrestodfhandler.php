...
class eZRESTODFHandler extends eZRESTBaseHandler
{
    ...
    public function initialize()
    {
        ...
        $moduleDefinition->addView( 'ezodfGetTopNodeList', ['method' => 'ezodfGetTopNodeList', 'functions' => 'ezodf_oo_client'] );
        $moduleDefinition->addView( 'ezodfGetChildren', ['method' => 'ezodfGetChildren', 'functions' => 'ezodf_oo_client', 'getParams' => ['nodeID'], 'getOptions' => ['languageCode' => false, 'offset' => 0, 'limit' => 10]] );
        $moduleDefinition->addView( 'ezodfGetMenuChildren', ['method' => 'ezodfGetMenuChildren', 'functions' => 'ezodf_oo_client', 'getParams' => ['nodeID'], 'getOptions' => ['languageCode' => false, 'offset' => 0, 'limit' => 10]] );
        $moduleDefinition->addView( 'ezodfGetNodeInfo', ['method' => 'ezodfGetNodeInfo', 'functions' => 'ezodf_oo_client', 'getParams' => ['nodeID'], 'getOptions' => ['languageCode' => false]] );
        // Alex 2008/04/16 - Added format argument: 'odt' (default), 'doc'
        $moduleDefinition->addView( 'ezodfFetchOONode', ['method' => 'ezodfFetchOONode', 'functions' => 'ezodf_oo_client', 'getParams' => ['nodeID'], 'getOptions' => ['languageCode' => false, 'base64Encoded' => true, 'format' => 'odt']] );
        // Alex 2008/04/22 - Added format argument: 'odt' (default), 'doc'
        $moduleDefinition->addView( 'ezodfPutOONode', ['method' => 'ezodfPutOONode', 'functions' => 'ezodf_oo_client', 'postParams' => ['nodeID', 'data'], 'postOptions' => ['languageCode' => false, 'base64Encoded' => true, 'format' => 'odt']] );
        // Alex 2008/04/22 - Added format argument: 'odt' (default), 'doc'
        $moduleDefinition->addView( 'ezodfReplaceOONode', ['method' => 'ezodfReplaceOONode', 'functions' => 'ezodf_oo_client', 'postParams' => ['nodeID', 'data'], 'postOptions' => ['languageCode' => null, 'base64Encoded' => true, 'format' => 'odt']] );

        // Add access functions for eZRESTODFHandler
        $moduleDefinition->addFunction( 'ezodf_oo_client', [] );
    ...
    public function ezodfFetchOONode( $getParams, $getOptions, $postParams, $postOptions )
    {
        ...
        match ($format) {
            'doc' => $ooElement->appendChild( $this->createDocDOMElement( $domDocument, $node ) ),
            'odt' => $ooElement->appendChild( $this->createOODOMElement( $domDocument, $node ) ),
            default => $ooElement,
        };
    ...
    public function ezodfGetChildren( $getParams, $getOptions, $postParams, $postOptions )
    {
        ...
        $children = eZContentObjectTreeNode::subTreeByNodeID( ['Depth' => 1, 'DepthOperator' => 'eq', 'Limit' => $limit, 'Offset' => $offset, 'Language' => $languageCode],
                                                              $nodeID );
        if ( $children )
        {
        ...
        {
            $childrenElement->setAttribute( 'count', count( (array) $children ) );
        }
    ...
    public function ezodfGetMenuChildren( $getParams, $getOptions, $postParams, $postOptions )
    {
        ...
        // Get children, and add them to children list.
        $children = eZContentObjectTreeNode::subTreeByNodeID( ['Depth' => 1, 'DepthOperator' => 'eq', 'Limit' => $limit, 'Offset' => $offset, 'Language' => $languageCode, 'ClassFilterType' => 'include', 'ClassFilterArray' => eZINI::instance( 'contentstructuremenu.ini' )->variable( 'TreeMenu', 'ShowClasses' )],
                                                              $nodeID );
        if ( $children )
        {
            foreach( $children as $childNode )
    ...
    protected function createTreeNodeDOMElement( DOMDocument $domDocument, eZContentObjectTreeNode $node )
    {
        ...
        $conditions = ['Depth' => 1, 'DepthOperator' => 'eq'];
    ...
    protected function importOODocument( $getParams, $getOptions, $postParams, $postOptions, $importType = 'import' )
    {
        ...
        if ( $base64Encoded )
        {
            ...
            $data = base64_decode( str_replace( ' ', '+', (string) $data ) );
        }

        // Store data to temporary file.
        // Alex 2008/04/22 - Added format argument: 'odt' (default), 'doc'
        $filename = substr( md5( random_int(0, mt_getrandmax()) ), 0, 8 ) . ".{$format}";
        $tmpFilePath = eZSys::cacheDirectory() . '/ezodf/' . substr( md5( random_int(0, mt_getrandmax()) ), 0, 8 );
