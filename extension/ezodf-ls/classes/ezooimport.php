...
class eZOOImport
{
    final public const ERROR_NOERROR = 0;
    final public const ERROR_UNSUPPORTEDTYPE = 1;
    final public const ERROR_PARSEXML = 2;
    final public const ERROR_OPENSOCKET = 3;
    final public const ERROR_CONVERT = 4;
    final public const ERROR_DAEMONCALL = 5;
    final public const ERROR_DAEMON = 6;
    final public const ERROR_DOCNOTSUPPORTED = 7;
    final public const ERROR_FILENOTFOUND = 8;
    final public const ERROR_PLACEMENTINVALID = 9;
    final public const ERROR_CANNOTSTORE = 10;
    final public const ERROR_UNKNOWNNODE = 11;
    final public const ERROR_ACCESSDENIED = 12;
    final public const ERROR_IMPORTING = 13;
    final public const ERROR_UNKNOWNCLASS = 14;
    final public const ERROR_UNKNOWN = 127;

    final public const NAMESPACE_OFFICE = 'urn:oasis:names:tc:opendocument:xmlns:office:1.0';
    final public const NAMESPACE_TEXT = 'urn:oasis:names:tc:opendocument:xmlns:text:1.0';
    final public const NAMESPACE_STYLE = 'urn:oasis:names:tc:opendocument:xmlns:style:1.0';
    final public const NAMESPACE_TABLE = 'urn:oasis:names:tc:opendocument:xmlns:table:1.0';
    final public const NAMESPACE_DRAWING = 'urn:oasis:names:tc:opendocument:xmlns:drawing:1.0';
    final public const NAMESPACE_SVG_COMPATIBLE = 'urn:oasis:names:tc:opendocument:xmlns:svg-compatible:1.0';
    final public const NAMESPACE_FO = 'urn:oasis:names:tc:opendocument:xmlns:xsl-fo-compatible:1.0';
    final public const NAMESPACE_XLINK = 'http://www.w3.org/1999/xlink';
    ...
    */
    function setError( $errorNumber = 0, $errorDescription = "" )
    {
        ...
        $this->ERROR['value'] = match ($errorNumber) {
            self::ERROR_NOERROR => "",
            self::ERROR_UNSUPPORTEDTYPE => ezpI18n::tr( 'extension/ezodf/import/error', "File extension or type is not allowed." ),
            self::ERROR_PARSEXML => ezpI18n::tr( 'extension/ezodf/import/error', "Could not parse XML." ),
            self::ERROR_OPENSOCKET => ezpI18n::tr( 'extension/ezodf/import/error', "Can not open socket. Please check if extension/ezodf/daemon.php is running." ),
            self::ERROR_CONVERT => ezpI18n::tr( 'extension/ezodf/import/error', "Can not convert the given document." ),
            self::ERROR_DAEMONCALL => ezpI18n::tr( 'extension/ezodf/import/error', "Unable to call daemon. Fork can not create child process." ),
            self::ERROR_DAEMON => ezpI18n::tr( 'extension/ezodf/import/error', "Daemon reported error." ),
            self::ERROR_UNKNOWNNODE => ezpI18n::tr( 'extension/ezodf/import/error', "Unknown node." ),
            self::ERROR_ACCESSDENIED => ezpI18n::tr( 'extension/ezodf/import/error', "Access denied." ),
            self::ERROR_IMPORTING => ezpI18n::tr( 'extension/ezodf/import/error', "Error during import." ),
            self::ERROR_UNKNOWNCLASS => ezpI18n::tr( 'extension/ezodf/import/error', "Unknown content class specified in odf.ini:" ),
            default => ezpI18n::tr( 'extension/ezodf/import/error', "Unknown error." ),
        };
    }
    ...
    function import( $file, $placeNodeID, $originalFileName, $importType = "import", $upload = null, $locale = false )
    {
        ...
        if ( $access['result'] )
        {
            ...
            if ( $importType !== "replace" )
            {
                ...
                $nodeAssignment = eZNodeAssignment::create( ['contentobject_id' => $object->attribute( 'id' ), 'contentobject_version' => $object->attribute( 'current_version' ), 'parent_node' => $parentNodeID, 'is_main' => 1]
                                                             );
                            ...
                            if ( is_object( $sectionNodeHash[$sectionName] ) )
                            {
                                ...
                                foreach ( $sectionNodeHash[$sectionName]->childNodes as $paragraph )
                                {
                                    ...
                                    foreach ( $paragraph->childNodes as $frame )
                                    {
                                        ...
                                        {
                                            $fileName = ltrim( (string) $imageNode->getAttributeNS( self::NAMESPACE_XLINK, 'href' ), '#' );
            ...
            foreach ( $this->RelatedImageArray as $image )
            {
                ...
                $nodeAssignment = eZNodeAssignment::create( ['contentobject_id' => $image['ID'], 'contentobject_version' => 1, 'parent_node' => $imageRootNode, 'is_main' => 1]
                                                             );
                $nodeAssignment->store();

                eZOperationHandler::execute( 'content', 'publish', ['object_id' => $image['ID'], 'version' => 1] );
...
//    if ( $node->namespaceURI() == 'http://openoffice.org/2000/text' )
        {
            ...
            switch ( $node->localName )
            {
                ...
                case 'p' :
                {
                    ...
                    if ( str_starts_with((string) $styleName, "eZCustom_20_") )
                    {
                        $customName = substr( (string) $styleName, 12, strlen( (string) $styleName ) - 12 );
                        $this->CollapsingTagName = "custom";
                        $this->CollapsingTagAttribute = "name='$customName'";
                    }
                    else
                    {
                        $this->CollapsingTagName = match ($styleName) {
                            "Preformatted_20_Text" => "literal",
                            default => false,
                        };
                    }
                    ...
                    foreach ( $this->AutomaticStyles as $style )
                    {
                        ...
                        if ( $styleName == $tmpStyleName )
                        {
                            ...
                            if ( str_starts_with((string) $parentStyleName, "Heading_20_") )
                            {
                                $level = substr( (string) $parentStyleName, 11, strlen( (string) $parentStyleName )  );
                                if ( is_numeric( $level ) )
                                    $headerLevel = $level;
                            }
                    ...
                    if ( str_starts_with((string) $styleName, "Heading_20_") )
                    {
                        $level = substr( (string) $styleName, 11, strlen( (string) $styleName )  );
                        if ( is_numeric( $level ) )
                            $headerLevel = $level;
                    }
                    ...
                    else
                    {
                        ...
                        if ( $this->CollapsingTagName == false )
                        {
                            ...
                            if ( $lastCollapsingTagName !== false )
                            {
                                ...
                                $tagContent = str_replace( "&", "&amp;", (string) $this->CollapsingTagContent );
                ...
                case 'numbered-paragraph' :
                {
                    ...
                    foreach ( $node->childNodes as $itemNode )
                    {
                        ...
                        {
                            $listContent .= "<li>" . strip_tags( (string) self::handleNode( $itemNode, $sectionLevel ) ) . "</li>";
                        }
                    }
                    ...
                    foreach ( $node->childNodes as $itemNode )
                    {
                        if ( $itemNode->localName == 'list-item' )
                        {
                            foreach ( $itemNode->childNodes as $childNode )
                            {
                                ...
                                if ( str_starts_with((string) $listItemContent, "<ol>") or
                                     str_starts_with((string) $listItemContent, "<ul>") )
                                {
            ...
            case "frame":
            {
                ...
                foreach ( $childNode->childNodes as $imageNode )
                {
                    switch ( $imageNode->localName )
                    {
                        case "image" :
                        {
                            $href = ltrim( (string) $imageNode->getAttributeNS( self::NAMESPACE_XLINK, 'href' ), '#' );
                            ...
                            foreach ( $this->AutomaticStyles as $style )
                            {
                                ...
                                if ( $styleName == $tmpStyleName )
                                {
                                    ...
                                    $imageAlignment = match ($alignment) {
                                        "left" => "left",
                                        "right" => "right",
                                        default => "center",
                                    };
...
*/
                                ...
                                $this->RelatedImageArray[] = ["ID" => $contentObjectID, "ContentObject" => $contentObject];
            ...
            case "#text" :
            {
                $tagContent = str_replace( "&", "&amp;", (string) $childNode->textContent );
                $tagContent = str_replace( ">", "&gt;", $tagContent );
                $tagContent = str_replace( "<", "&lt;", $tagContent );
                $tagContent = str_replace( "'", "&apos;", $tagContent );
                $tagContent = str_replace( '"', "&quot;", $tagContent );
            ...
            case "span" :
            {
                ...
                if ( str_starts_with((string) $styleName, "eZCustominline_20_") )
                    $inlineCustomTagName = substr( (string) $styleName, 18 );
    ...
    function createSubNode( $node, $name )
    {
        ...
        if ( empty( $namedChildrenArray ) )
        {
            $class = eZContentClass::fetchByIdentifier( "folder" );
            {
                ...
                $nodeAssignment = eZNodeAssignment::create( ['contentobject_id' => $contentObject->attribute( 'id' ), 'contentobject_version' => $contentObject->attribute( 'current_version' ), 'parent_node' => $node->attribute( 'node_id' ), 'is_main' => 1]
                                                             );
                $nodeAssignment->store();
        ...
        {
            if ( (is_countable($namedChildrenArray) ? count( $namedChildrenArray ) : 0) == 1 )
            {
    ...
    */
    ...
    public $RelatedImageArray = [];
    public $AutomaticStyles = [];
    public $ImportDir = "var/cache/ezodf/import/";
    public $ImportBaseDir = "var/cache/ezodf/import/";
    public $InsideListType = false;
    ...
    public $CollapsingTagName = false;
    public $CollapsingTagContent = false;
    public $CollapsingTagAttribute = false;
