...
{
    final public const DATA_TYPE_STRING = "ezpaex";
    ...
    function validateObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        if ( $statusPasswordlifetime != eZInputValidator::STATE_ACCEPTED && trim( (string) $passwordlifetime ) )
        {
        ...
        if ( $statusExpirationnotification != eZInputValidator::STATE_ACCEPTED && trim( (string) $expirationnotification ) )
        {
            $contentObjectAttribute->setValidationError( ezpI18n::tr( 'mbpaex/classes/datatypes',
        ...
        if ( $paex->isUser() )
        {
            ...
            if (trim((string) $newPassword) && ($newPassword != "_ezpassword"))
            {
                if ( $paex->canEdit() )
                {
        ...
        else // If we are updating a group, try to check the updatechildren checkbox
        {
            ...
            if ( $statusUpdatechildren != eZInputValidator::STATE_ACCEPTED && trim((string) $updatechildren) )
            {
                $contentObjectAttribute->setValidationError( ezpI18n::tr( 'mbpaex/classes/datatypes',
        ...
        if ( $paex->isUser() )
        {
            ...
            if ( trim( (string) $newPassword ) && ( $newPassword != "_ezpassword" ) )
            {
                ...
                if ( $currentUserID == $contentObjectID )
                {
                    ...
                    eZAudit::writeAudit( 'user-password-change-self', [] );
                }
                ...
                else
                {
                    ...
                    eZAudit::writeAudit( 'user-password-change', ['User id' => $targetUser->attribute( 'contentobject_id' ), 'User login' => $targetUser->attribute( 'login' )] );
                }
