...
if ( $http->hasPostVariable( "OKButton" ) && $user)
{
    ...
    if ( $user->authenticateHash( $login, $oldPassword, $site, $type, $hash ) ) // Old password is correct
    {
        if (  $newPassword == $confirmPassword )
        {
            if ( !$user->validatePassword($newPassword) )
            {
                ...
                eZAudit::writeAudit( 'user-password-change-self-fail', ['UserID: ' => $UserID, 'Login: ' => $login, 'Comment: ' => 'Password not pass standard validation'] );
            ...
            else
            {
                ...
                {
                    // if audit is enabled password changes should be logged
                    eZAudit::writeAudit( 'user-password-change-self-fail', ['UserID: ' => $UserID, 'Login: ' => $login, 'Comment: ' => 'Password not pass PAEX validation'] );
                    $newPasswordNotValidate = 1;
                }
                else
                {
                    ...
                    {
                        // if audit is enabled password changes should be logged
                        eZAudit::writeAudit( 'user-password-change-self-fail', ['UserID: ' => $UserID, 'Login: ' => $login, 'Comment: ' => 'New and old password are the same'] );
                        $newPasswordMustDiffer = 1;
                    }
                    else
                    {
                        // if audit is enabled password changes should be logged
                        eZAudit::writeAudit( 'user-password-change-self', ['UserID: ' => $UserID, 'Login: ' => $login] );
        ...
        {
            // if audit is enabled password changes should be logged
            eZAudit::writeAudit( 'user-password-change-self-fail', ['UserID: ' => $UserID, 'Login: ' => $login, 'Comment: ' => 'Password not match password confirmation'] );
            $newPassword = "";
            $confirmPassword = "";
    ...
    {
        // if audit is enabled password changes should be logged
        eZAudit::writeAudit( 'user-password-change-self-fail', ['UserID: ' => $UserID, 'Login: ' => $login, 'Comment: ' => 'Old password incorrect'] );
        $oldPassword = "";
