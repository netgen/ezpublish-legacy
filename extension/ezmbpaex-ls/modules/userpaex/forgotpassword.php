...
if ( strlen( (string) $hashKey ) == 32 )
{
    ...
    if (!is_object($forgotPasswdObj)) // HashKey not found
    {
        ...
        eZAudit::writeAudit( 'user-forgotpassword-fail', ['HashKey' => $hashKey, 'Comment' => 'HashKey not found'] );
    ...
    {
        // if audit is enabled password changes should be logged
        eZAudit::writeAudit( 'user-forgotpassword-fail', ['HashKey' => $hashKey, 'Comment' => 'HashKey expired, proceed to remove'] );
...
if ( $module->isCurrentAction( "Generate" ) )
{
    ...
    if ( $module->hasActionParameter( "Email" ) && eZMail::validate($module->actionParameter( "Email" )) )
    {
        ...
        if ( !empty( $users ) )
        {
            $user = $users[0];
            $time = time();
            $hashKey = md5( $time . ":" . random_int(0, mt_getrandmax()) );
            ...
            // if audit is enabled password changes should be logged
            eZAudit::writeAudit( 'user-forgotpassword', ['Email' => $email, 'Comment' => 'Forgotpassword email sent'] );

        }
        else
        {
            // if audit is enabled password changes should be logged
            eZAudit::writeAudit( 'user-forgotpassword-fail', ['Email' => $email, 'Comment' => 'Email address not found'] );
...
else if ( $module->isCurrentAction( "ChangePassword" ) && $hashKeyValidated )
{
    ...
    if (  $newPassword ==  $confirmPassword )
    {
        ...
        {
            // if audit is enabled password changes should be logged
            eZAudit::writeAudit( 'user-forgotpassword-fail', ['UserID' => $UserID, 'Login' => $login, 'Comment: ' => 'Password not pass standard validation'] );
        ...
        else
        {
            ...
            {
                // if audit is enabled password changes should be logged
                eZAudit::writeAudit( 'user-forgotpassword-fail', ['UserID' => $UserID, 'Login' => $login, 'Comment: ' => 'Password not pass PAEX validation'] );
            ...
            else
            {
                ...
                {
                    // if audit is enabled password changes should be logged
                    eZAudit::writeAudit( 'user-forgotpassword-fail', ['UserID' => $UserID, 'Login' => $login, 'Comment: ' => 'New and old password are the same'] );
                ...
                {
                    // if audit is enabled password changes should be logged
                    eZAudit::writeAudit( 'user-forgotpassword', ['UserID' => $UserID, 'Login' => $login, 'Comment: ' => 'Password changed successfully'] );
    ...
    {
        // if audit is enabled password changes should be logged
        eZAudit::writeAudit( 'user-forgotpassword-fail', ['UserID' => $UserID, 'Login' => $login, 'Comment: ' => 'Password not match password confirmation'] );
