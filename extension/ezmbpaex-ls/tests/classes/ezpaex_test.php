...
class eZPaExTest extends ezpDatabaseTestCase
{
    ...
    public function setUp()
    {
        ...
        $row = ['contentobject_id' => 14, 'passwordvalidationregexp' => '', 'passwordlifetime' => 3, 'expirationnotification' => eZPaEx::NOT_DEFINED, 'password_last_updated' => $this->paexTime, 'updatechildren' => 0, 'expirationnotification_sent' => 0];
    ...
    **/
    ...
    {
        static::assertInstanceOf('eZPaEx', $this->paex);

        static::assertEquals(14, $this->paex->attribute( 'contentobject_id' ));
        static::assertEquals('', $this->paex->attribute( 'passwordvalidationregexp' ));
        static::assertEquals(3, $this->paex->attribute( 'passwordlifetime' ));
        static::assertEquals(eZPaEx::NOT_DEFINED, $this->paex->attribute( 'expirationnotification' ));
        static::assertEquals($this->paexTime, $this->paex->attribute( 'password_last_updated' ));
        static::assertEquals(0, $this->paex->attribute( 'updatechildren' ));
        static::assertEquals(0, $this->paex->attribute( 'expirationnotification_sent' ));
    }
    ...
    **/
    public function testCreate()
    {
        ...
        static::assertInstanceOf('eZPaEx', $paex);
        static::assertEquals($this->paexINI->variable( 'mbpaexSettings', 'PasswordValidationRegexp' ), $paex->attribute( 'passwordvalidationregexp' ), "Password validation regexp value doesn't match");
        static::assertEquals($this->paexINI->variable( 'mbpaexSettings', 'DefaultPasswordLifeTime' ), $paex->attribute( 'passwordlifetime' ), "Password lifetime value doesn't match");
        static::assertEquals($this->paexINI->variable( 'mbpaexSettings', 'ExpirationNotification' ), $paex->attribute( 'expirationnotification' ), "Password expiration notification value doesn't match");
        static::assertEquals($user->attribute( 'contentobject_id' ), $paex->attribute( 'contentobject_id' ), "Paex contentobject ID doesn't match");
    }
    ...
    **/
    public function testSetInformation()
    {
        ...
        static::assertEquals($contentObjectID, $paex->attribute( 'contentobject_id' ), "attribute contentobject_id doesn't match");
        static::assertEquals($regexp, $paex->attribute( 'passwordvalidationregexp' ), "attribute passwordvalidationregexp doesn't match");
        static::assertEquals($passwordLifetime, $paex->attribute( 'passwordlifetime' ), "attribute passwordlifetime doesn't match");
        static::assertEquals($expirationNotification, $paex->attribute( 'expirationnotification' ), "attribute expirationnotification doesn't match");
        static::assertEquals($time, $paex->attribute( 'password_last_updated' ), "attribute password_last_updated doesn't match");
        static::assertEquals($updateChildren, $paex->attribute( 'updatechildren' ), "attribute updatechildren doesn't match");
        static::assertEquals($expirationNotificationSent, $paex->attribute( 'expirationnotification_sent' ), "attribute expirationnotification_sent doesn't match");
    }
    ...
    **/
    public function testStore()
    {
        ...
        // @expected null
        static::assertNull(eZPaEx::fetch( $contentObjectID ), "PaEx #$contentObjectID hasn't been stored yet, fetch() should have returned null");

        // store, and try to fetch again
        $paex->store();
        $fetchedPaex = eZPaEx::fetch( $contentObjectID );
        static::assertInstanceOf('eZPaEx', $fetchedPaex, "PaEx #$contentObjectID has been stored, fetch() should have returned the object");
    }
    ...
    **/
    public function testRemovePaex()
    {
        ...
        static::assertInstanceOf('eZPaEx', eZPaEx::fetch( $id ), "eZPaEx::fetch() should have returned an object");

        // remove, and check if fetch fails as expected
        eZPaEx::removePaex( $id );
        static::assertEquals(null, eZPaEx::fetch( $id ), "eZPaEx::fetch() should have returned null as the object should no longer exist");
    }
    ...
    **/
    public function testFetch()
    {
        ...
        static::assertNull(eZPaEx::fetch( $contentObjectID ), "eZPaEx::fetch() should be null as the object hasn't been created yet");

        // Create & store the PaEx
        eZPaEx::create( $contentObjectID )->store();

        // Try fetching it again
        static::assertInstanceOf('eZPaEx', eZPaEx::fetch( $contentObjectID ));
    }
    ...
    public function testIsExpired( $passwordLifetime, $passwordLastUpdated, $expected )
    {
        ...
        static::assertEquals($expected, $this->paex->isExpired());
    }
    ...
    {
        static::assertTrue($this->paex->isUser(), "PaEx object should have been a user");
        
        $paex = new eZPaEx( [] );
        static::assertFalse($paex->isUser(), "PaEx object shouldn't have been a user");
    }
    ...
    function testHasRegexp()
    {
        ...
        static::assertFalse($this->paex->hasRegexp(), "PaEx object shouldn't have a regexp");
        
        // Set a regexp and test again
        $this->paex->setAttribute( 'passwordvalidationregexp', '/^[a-z0-9]$/' );
        static::assertTrue($this->paex->hasRegexp(), "PaEx object should have a regexp");
    }
    ...
    {
        static::assertTrue($this->paex->hasLifeTime(), "PaEx object should have a lifetime");
        
        // Set lifetime to an empty value
        $this->paex->setAttribute( 'passwordlifetime', '' );
        static::assertFalse($this->paex->hasLifeTime(), "PaEx object shouldn't have a lifetime (empty string)");
        
        // Set lifetime to the NOT_DEFINED constant
        $this->paex->setAttribute( 'passwordlifetime', eZPaEx::NOT_DEFINED );
        static::assertFalse($this->paex->hasLifeTime(), "PaEx object shouldn't have a lifetime (eZPaEx::NOT_DEFINED)");
    }
    ...
    function testHasNotification()
    {
        ...
        static::assertFalse($this->paex->hasNotification(), "PaEx object should not have a lifetime");
        ...
        static::assertTrue($this->paex->hasNotification(), "PaEx object should have a lifetime");
    }
    ...
    function testValidatePassword( $regexp, $expected, $password = 'publish' )
    {
        ...
        if ( $expected === true )
        {
            static::assertTrue($this->paex->validatePassword( $password ));
        }
        elseif ( $expected === false )
        {
            static::assertFalse($this->paex->validatePassword( $password ));
        }
        else
        {
            static::markTestSkipped("Unknown value for \$expected");
        }
    }
