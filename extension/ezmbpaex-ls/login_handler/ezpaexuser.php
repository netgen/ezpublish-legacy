...
class eZPaExUser extends eZUser
{
    ...
    protected static function passwordHasExpired( $user )
    {
        ...
        $userIDAudit = $userID ?? 'null';
        $loginEscaped = eZDB::instance()->escapeString( $user->attribute( 'login' ) );
        eZAudit::writeAudit( 'user-failed-login', ['User id' => $userIDAudit, 'User login' => $loginEscaped, 'Comment' => 'Failed login attempt: Password Expired. eZPaExUser::loginUser()'] );
