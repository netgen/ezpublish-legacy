...
class ezsrRatingObject extends eZPersistentObject
{
    ...
    {
        static $def = ['fields' => ['contentobject_id' => ['name' => 'contentobject_id', 'datatype' => 'integer', 'default' => 0, 'required' => true, 'foreign_class' => 'eZContentObject', 'foreign_attribute' => 'id', 'multiplicity' => '1..*'], 'contentobject_attribute_id' => ['name' => 'contentobject_attribute_id', 'datatype' => 'integer', 'default' => 0, 'required' => true, 'foreign_class' => 'eZContentObjectAttribute', 'foreign_attribute' => 'id', 'multiplicity' => '1..*'], 'rating_average' => ['name' => 'rating_average', 'datatype' => 'float', 'default' => 0, 'required' => false], 'rating_count' => ['name' => 'rating_count', 'datatype' => 'integer', 'default' => 0, 'required' => false]], 'keys' => ['contentobject_id', 'contentobject_attribute_id'], 'function_attributes' => ['rounded_average' => 'getRoundedAverage', 'rating' => 'getRating', 'rating_data' => 'getRatingData', 'current_user_has_rated' => 'currentUserHasRated'], 'class_name' => 'ezsrRatingObject', 'name' => 'ezstarrating'];
        return $def;
    }
    ...
    {
        $rateDataObj = ezsrRatingDataObject::create( ['contentobject_id' => $this->attribute('contentobject_id'), 'contentobject_attribute_id' =>  $this->attribute('contentobject_attribute_id'), 'created_at' => 0] );
    ...
    {
        $custom = [['operation' => 'count( id )', 'name'      => 'rating_count'], ['operation' => 'avg( rating )', 'name'      => 'rating_average']];
        $cond = ['contentobject_id' => $this->attribute('contentobject_id'), 'contentobject_attribute_id' => $this->attribute('contentobject_attribute_id')];
        $data = self::fetchObjectList( ezsrRatingDataObject::definition(), [], $cond, null, null, false, false, $custom );
    ...
    static function stats( $ContentObjectID )
    {
        ...
        {
            $custom = [['operation' => 'sum( rating_count )', 'name'      => 'rating_count'], ['operation' => 'avg( rating_average )', 'name'      => 'rating_average']];
            $cond = ['contentobject_id' => $ContentObjectID];
            $return = self::fetchObjectList( self::definition(), [] ,$cond, null, null, false, false, $custom );
    ...
    static function fetchNodeByRating( $params )
    {
        ...
        $limit       = false;
        $fromSql     = '';
        $asObject         = $params['as_object'] ?? true;
        $loadDataMap      = $params['load_data_map'] ?? false;
        $mainNodeOnly     = $params['main_node_only'] ?? false;
        $ignoreVisibility = $params['ignore_visibility'] ?? false;
        $classFilterType  = $params['class_filter_type'] ?? false;
        $classFilterArray = $params['class_filter_array'] ?? false;
        $includeNotRated  = $params['include_not_rated'] ?? false;
        ...
        {
            $orderBySql = 'ORDER BY ';
            $orderArr = is_string( $params['sort_by'][0] ) ? [$params['sort_by']] : $params['sort_by'];
        ...
        if ( $offset !== false || $limit !== false )
            $rows = $db->arrayQuery( $sql, ['offset' => $offset, 'limit' => $limit], $server );
        else
