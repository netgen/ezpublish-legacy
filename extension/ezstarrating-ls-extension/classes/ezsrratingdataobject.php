...
class ezsrRatingDataObject extends eZPersistentObject
{
    ...
    {
        static $def = ['fields' => ['id' => ['name' => 'id', 'datatype' => 'integer', 'default' => 0, 'required' => true], 'created_at' => ['name' => 'created_at', 'datatype' => 'integer', 'default' => 0, 'required' => true], 'user_id' => ['name' => 'user_id', 'datatype' => 'integer', 'default' => 0, 'required' => true, 'foreign_class' => 'eZUser', 'foreign_attribute' => 'contentobject_id', 'multiplicity' => '0..1'], 'session_key' => ['name' => 'session_key', 'datatype' => 'string', 'default' => '', 'required' => true], 'rating' => ['name' => 'rating', 'datatype' => 'integer', 'default' => 0, 'required' => true], 'contentobject_id' => ['name' => 'contentobject_id', 'datatype' => 'integer', 'default' => 0, 'required' => true, 'foreign_class' => 'eZContentObject', 'foreign_attribute' => 'id', 'multiplicity' => '1..*'], 'contentobject_attribute_id' => ['name' => 'contentobject_attribute_id', 'datatype' => 'integer', 'default' => 0, 'required' => true, 'foreign_class' => 'eZContentObjectAttribute', 'foreign_attribute' => 'id', 'multiplicity' => '1..*']], 'keys' => ['id'], 'function_attributes' => ['average_rating' => 'getAverageRating'], 'increment_key' => 'id', 'class_name' => 'ezsrRatingDataObject', 'name' => 'ezstarrating_data'];
        return $def;
    }
    ...
    function getAverageRating()
    {
        ...
        {
            $avgRating = ezsrRatingObject::create( ['contentobject_id' => $this->attribute('contentobject_id'), 'contentobject_attribute_id' => $this->attribute('contentobject_attribute_id')] );
        }
        return $avgRating;
    }
    ...
    function userHasRated( $returnRatedObject = false )
    {
        if ( $this->currentUserHasRated === null )
        {
            ...
            if ( $http->hasSessionVariable( 'ezsrRatedAttributeIdList', $useUserSession ) )
                $attributeIdList = explode( ',', (string) $http->sessionVariable('ezsrRatedAttributeIdList') );
            else
                $attributeIdList = [];
    ...
    static function rate( $contentobjectId, $contentobjectAttributeId, $rate )
    {
        ...
        if ( is_numeric( $contentobjectId ) &&
             ...
             $rate <= 5 && $rate > 0 )
        {
            $row = ['contentobject_attribute_id' => $contentobjectAttributeId, 'contentobject_id'           => $contentobjectId, 'rating'                     => $rate];
    ...
    static function fetchByConds( $params )
    {
        ...
        {
            if ( !isset( $conds['contentobject_id'] ) ) $conds['contentobject_id'] = ['!=', 0];// to make sure index is used
        ...
        {
            if ( !isset( $conds['user_id'] ) ) $conds['user_id'] = ['!=', 0];// to make sure index is used
            $conds['session_key'] = $params['session_key'];
        }

        if ( isset( $params['limit'] ) )
        {
            $limit = ['length' => $params['limit'], 'offset' => ($params['offset'] ?? 0 )];
        }
        ...
        if ( isset( $params['sort_by'] ) )
        {
            if ( isset( $params['sort_by'][1] ) && is_array( $params['sort_by'] ) )
                $sorts = [$params['sort_by'][0] => ( $params['sort_by'][1] ? 'asc' : 'desc' )];
            else
