...
class Cpdf
{
    ...
    {
        $this->currentColour = eZMath::rgbToCMYK( ['r' => -1, 'g' => -1, 'b' => -1] );
        $this->currentStrokeColour = eZMath::rgbToCMYK( ['r' => -1, 'g' => -1, 'b' => -1] );
    ...
    function o_pages( $id, $action, $options = '' )
    {
        ...
        case 'font':
            ...
            break;
        case 'shading':
            $o['info']['shading'][] = ['objNum' => $options['objNum'], 'label' => $options['label']];
            break;
        case 'xObject':
        ...
        case 'out':
            if ( is_countable($o['info']['pages']) ? count( $o['info']['pages'] ) : 0 )
            {
                ...
                if ( ( isset( $o['info']['fonts'] ) && (is_countable($o['info']['fonts']) ? count( $o['info']['fonts'] ) : 0) ) || isset( $o['info']['procset'] ) )
                {
                    ...
                    if ( isset( $o['info']['shading'] ) && (is_countable($o['info']['shading']) ? count( $o['info']['shading'] ) : 0) )
                    {
                        $res .= "\n/Shading <<";
    ...
    function o_function( $id, $action, $options = '' )
    {
        ...
        switch ( $action )
        {
            case 'new':
            {
                $this->objects[$id] = ['t' => 'function', 'info' => []];
    ...
    function o_pattern( $id, $action, $options = '' )
    {
        ...
        switch ( $action )
        {
            case 'new':
            {
                $this->objects[$id] = ['t' => 'shading', 'info' => []];
    ...
    function o_shading( $id, $action, $options = '' )
    {
        ...
        switch ( $action )
        {
            case 'new':
            {
                $this->objects[$id] = ['t' => 'shading', 'info' => ['type' => '2', 'ColorSpace' => '/DeviceCMYK']];
            ...
            if ( is_array( $options['color0'] ) ) //Set chading colors
            {
                ...
                $color0 = ['c' => sprintf( '%.3F', $options['color0']['c'] ), 'm' => sprintf( '%.3F', $options['color0']['m'] ), 'y' => sprintf( '%.3F', $options['color0']['y'] ), 'k' => sprintf( '%.3F', $options['color0']['k'] )];
            }
            else
            {
            ...
            if ( is_array( $options['color1'] ) )
            {
                ...
                $color1 = ['c' => sprintf( '%.3F', $options['color1']['c'] ), 'm' => sprintf( '%.3F', $options['color1']['m'] ), 'y' => sprintf( '%.3F', $options['color1']['y'] ), 'k' => sprintf( '%.3F', $options['color1']['k'] )];
            }
            else
            {
            ...
            $this->o_function( $this->numObj, 'new', ['type' => '2', 'Domain' => '[0.0 1.0]', 'C0' => '['.$color0['c'].' '.$color0['m'].' '.$color0['y'].' '.$color0['k'].']', 'C1' => '['.$color1['c'].' '.$color1['m'].' '.$color1['y'].' '.$color1['k'].']', 'N' => '1.0'] );
            $this->objects[$id]['info']['Extend'] = '[true true]';
            // also tell the pages node about the new shading
            $this->o_pages( $this->currentNode, 'shading', ['label' => 'Sh' . $id, 'objNum'=> $id] );
            return 'Sh' . $id;
            } break;
    ...
    function o_font( $id, $action, $options = '' )
    {
        ...
        {
        case 'new':
            $this->objects[$id] = ['t' => 'font', 'info' => ['name' => $options['name'], 'SubType' => 'Type1']];
 ...
 */
    ...
    function o_info( $id, $action, $options = '' )
    {
        ...
        case 'new':
            $this->infoObject = $id;
            $date = 'D:' . date( 'Ymd' );
            $this->objects[$id] = ['t' => 'info', 'info' => ['Creator' => 'eZ Publish CMS, http://ez.no', 'CreationDate' => $date]];
            break;
        case 'Title':
        case 'Author':
        case 'Subject':
 ...
 */
    ...
    switch ($action){
        ...
        if (strlen((string) $options) && intval($options)){
 ...
 */
    ...
    switch($action){
        ...
        $this->objects[$id]['info']['ColorSpace'] = match ($options['channels']) {
            1 => '/DeviceGray',
            default => '/DeviceRGB',
        };
        ...
        if (strlen((string) $options['pdata'])){
            $tmp = ' [ /Indexed /DeviceCMYK '.(strlen((string) $options['pdata'])/3-1).' ';
            $this->numObj++;
 ...
 */
    ...
    switch($action){
        ...
        $len = strlen((string) $options['owner']);
        if ($len>32){
        $owner = substr((string) $options['owner'],0,32);
        } else if ($len<32){
        $owner = $options['owner'].substr($pad,0,32-$len);
        } else {
        $owner = $options['owner'];
        }
        $len = strlen((string) $options['user']);
        if ($len>32){
        $user = substr((string) $options['user'],0,32);
        } else if ($len<32){
 ...
 */
    ...
    $key = $this->md5_16($tmp);
    $this->ARC4_init(substr((string) $key,0,10));
    }
...
//  $content="%PDF-1.3\n";
    ...
    if (strlen((string) $this->fileIdentifier)){
 ...
 * @access private
 */
    function newDocument($pageSize=[0, 0, 612, 792]){
    $this->numObj=0;
    $this->objects = [];
    ...
    function openFont( $font )
    {
        ...
        {
            $dir = substr( (string) $font, 0, $pos + 1 );
            $name = substr( (string) $font, $pos + 1 );
        }

        if ( str_ends_with((string) $name, '.font') )
        {
            $name = substr( (string) $name, 0, strlen( (string) $name ) - 5 );
        }
    ...
    function selectFont( $fontName, $encoding = '', $set = 1 )
    {
        if ( !isset( $this->fonts[$fontName] ) )
        {
            ...
            if ( isset( $this->fonts[$fontName] ) )
            {
                $this->numObj++;
                $this->numFonts++;
                $pos = strrpos( (string) $fontName, '/' );
                $name = substr( (string) $fontName, $pos + 1 );
                if ( str_ends_with($name, '.font') )
                {
                    $name = substr( $name, 0, strlen( $name ) - 5 );
                }
                if ( !isset( $this->fontFamilies[$name] ) )
                {
                    $this->setFontFamily( $name, ['b' => $name . '-Bold', 'i' => $name . '-Italic', 'bi' =>  $name . '-BoldItalic', 'ib' => $name . '-BoldItalic'] );
                }
                $options = ['name' => $name];
                ...
                else if ( strlen( (string) $encoding ) )
                {
                ...
                $basefile = substr( (string) $fontName, 0, strlen( (string) $fontName ) );
    ...
    */
    ...
    {
        if ( strlen( (string) $this->currentBaseFont ) == 0 )
        {
            // then assume an initial font
            $this->selectFont( 'lib/ezpdf/classes/fonts/Helvetica' );
        }
        $cf = substr( (string) $this->currentBaseFont, strrpos( (string) $this->currentBaseFont, '/' ) + 1 );
        if ( strlen( (string) $this->currentTextState )
             ...
             && isset( $this->fontFamilies[$cf][$this->currentTextState] ) )
        {
            ...
            $nf = substr( (string) $this->currentBaseFont, 0, strrpos( (string) $this->currentBaseFont, '/' ) + 1 ) .
    ...
    */
    ...
    function newPage( $insert = 0, $id = 0, $pos = 'after' )
    {
        ...
        if ( strlen( (string) $this->currentLineStyle ) )
        {
    ...
    function PRVTcheckTextDirective1( &$text, $i, &$f, $final, &$x, &$y, $size = 0, $angle = 0, $wordSpaceAdjust = 0 )
    {
        ...
        if ( $text[$j] == '<' )
        {
            ...
            case '/':
                ...
                switch ( $text[$j] )
                {
                    ...
                    {
                        if ( $final === true )
                            $this->currentTextState = substr( (string) $this->currentTextState, 0, $p).substr( (string) $this->currentTextState, $p+1);
                        $directive = $j-$i+1;
                    }
                    break;
                    ...
                    case 'c':
                    {
                        ...
                        if ( $k !== false && $text[$j] == ':' )
                        {
                            ...
                            $tmp = substr( (string) $text, $j+1, $k-$j-1 );
                            $b1 = strpos( $tmp, ':' );
                            if ( $b1 !== false )
                            {
                            ...
                            else
                            {
                                ...
                                if ( $final === 1 )
                                {
                                    ...
                                    $tmp = $this->PRVTgetTextPosition( $x, $y, $angle, $size, $wordSpaceAdjust, substr( (string) $text, $startInfo['i'], $i-$startInfo['i'] ) );
                ...
                if ( $k !== false && $text[$j] == ':' )
                {
                    ...
                    else
                    {
                        ...
                        if ($final === 1)
                        {
                            ...
                            $tmp = $this->PRVTgetTextPosition($x,$y,$angle,$size,$wordSpaceAdjust,substr((string) $text,0,$i));
                            ...
                            $info = ['x' => $tmp[0], 'y' => $tmp[1], 'angle' => $angle, 'status' => 'start', 'p' => $parm, 'f' => $func, 'height' => $this->getFontHeight( $size ), 'decender' => $this->getFontDecender( $size ), 'i' => $i];
        ...
        return ['directive' => $directive, 'y' => (float)$y, 'noClose' => $noClose, 'run_final' => ( $f == 0 )];
    }
    ...
    function addText( $x, $y, $size, $text, $angle = 0, $wordSpaceAdjust = 0 )
    {
        ...
        if ($this->nCallback>0)
        {
            ...
            {
                // call each function
                $info = ['x' => $x, 'y' => $y, 'angle' => $angle, 'status' => 'sol', 'p' => $this->callback[$i]['p'], 'nCallback' => $this->callback[$i]['nCallback'], 'height' => $this->callback[$i]['height'], 'decender' => $this->callback[$i]['decender']];
        ...
        for ( $i = 0; $i < $len; $i++ )
        {
            ...
            if ($directive)
            {
                ...
                if ( $i > $start )
                {
                    $part = substr((string) $text,$start,$i-$start);
        ...
        if ( $this->nCallback > 0 )
        {
            for ( $i = $this->nCallback; $i > 0; $i-- )
            {
                ...
                $info = ['x' => $tmp[0], 'y' => $tmp[1], 'angle' => $angle, 'status' => 'eol', 'p' => $this->callback[$i]['p'], 'nCallback' => $this->callback[$i]['nCallback'], 'height' => $this->callback[$i]['height'], 'decender' => $this->callback[$i]['decender']];
                $func = $this->callback[$i]['f'];
                $this->$func( $info );
            }
        }
    ...
    function addTextWrap( $x, $y, $width, $size, $text, $justification = 'left', $angle = 0, $test = 0 )
    {
        ...
        $returnArray = ['text' => '', 'width' => 0, 'height' => 0];
        if ( $text != '' )
        {
            ...
            else
            {
                ...
                if ( $w > $tw )
                {
                    ...
                    if ( $break > 0 )
                    {
                        ...
                        if ($text[$break] == ' ')
                        {
                            $tmp = substr( (string) $text, 0, $break );
                        }
                        else
                        {
                            $tmp = substr( (string) $text, 0, $break + 1 );
                        }
                        $adjust = 0;
                        ...
                        $returnArray['text'] = substr( (string) $text, $break + 1 );
                        return $returnArray;
                    }
                    else
                    {
                        // just split before the current character
                        $tmp = substr( (string) $text, 0, $i );
                        $adjust = 0;
                        $ctmp = ord( $text[$i] );
                        ...
                        $returnArray['text'] = substr( (string) $text, $i );
                        return $returnArray;
                    }
                }
    ...
    function addPngFromFile( $file, $x, $y, $w = 0, $h = 0 )
    {
        ...
        if ( !$error )
        {
            ...
            while ( $p < $len )
            {
                ...
                $chunkType = substr((string) $data,$p+4,4);
                ...
                case 'PLTE':
                    $pdata .= substr( (string) $data, $p + 8, $chunkLen );
                    break;
                case 'IDAT':
                    $idata .= substr( (string) $data, $p + 8, $chunkLen );
                    break;
                case 'tRNS':
    ...
    //  $this->o_image($this->numObj,'new',array('label'=>$label,'data'=>$idata,'iw'=>$w,'ih'=>$h,'type'=>'png','ic'=>$info['width']));
        $options = ['label' => $label, 'data' => $idata, 'bitsPerComponent' => $info['bitDepth'], 'pdata' => $pdata, 'iw' => $info['width'], 'ih' => $info['height'], 'type' => 'png', 'color' => $color, 'ncolor' => $ncolor];
    ...
    function setFontFamily($family,$options='')
    {
        if (!is_array($options))
        {
            if ($family=='init')
            {
                ...
                $this->fontFamilies['Helvetica'] = ['b'=>'Helvetica-Bold', 'i'=>'Helvetica-Oblique', 'bi'=>'Helvetica-BoldOblique', 'ib'=>'Helvetica-BoldOblique'];
                $this->fontFamilies['Courier'] = ['b'=>'Courier-Bold', 'i'=>'Courier-Oblique', 'bi'=>'Courier-BoldOblique', 'ib'=>'Courier-BoldOblique'];
                $this->fontFamilies['Times-Roman'] = ['b'=>'Times-Bold', 'i'=>'Times-Italic', 'bi'=>'Times-BoldItalic', 'ib'=>'Times-BoldItalic'];
            }
        }
        else
        {
