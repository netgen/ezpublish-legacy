...
$scriptOptions = $script->getOptions( "[repository:][package:][package-dir:][url:][admin-siteaccess:][user-siteaccess:][auto-mode:]",
                                      ...
                                      ['repository' => "Path to repository where unpacked(unarchived) packages are \n" .
                                               ...
                                               "if 'package-dir' is not specified then default dir('/tmp/ezwebin') will be used.", 'admin-siteaccess' => 'Will be used as base for eZWebin admin siteaccess', 'user-siteaccess' => 'Will be used as base for eZWebin user siteaccesses', 'auto-mode' => "[on/off]. Do not ask what to do in case of confilicts. By default is 'on'"],
                                      false,
                                      ['user' => true]
                                     );
...
// 'package' option
//
$packageList = ['ezwebin_classes', 'ezwebin_extension', 'ezwebin_banners', 'ezwebin_democontent', 'ezwebin_design', 'ezwebin_site'];
...
if( downloadPackages( $packageList, $packageURL, $packageDir, $packageRepository ) )
{
    if( file_exists( installScriptDir( $packageRepository ) ) )
    {
        ...
        //
        $params = ['object_remote_map' => ['1bb4fe25487f05527efa8bfd394cecc7' => 14, '5f7f0bdb3381d6a461d8c29ff53d908f' => 11, '15b256dbea2ae72418ff5facc999e8f9' => 42], 'package_object' => $package, 'design_list' => ['ezwebin_site', 'admin'], 'user_siteaccess' => 'ezwebin_site', 'admin_siteaccess' => 'ezwebin_site_admin', 'site_type' => ['access_type' => $accessType, 'access_type_value' => $accessTypeValue, 'admin_access_type_value' => $adminAccessTypeValue], 'all_language_codes' => $locales, 'primary_language' => $primaryLanguage->attribute( 'locale' ), 'host' => $siteINI->variable( 'SiteSettings', 'SiteURL' )];

        $user = eZUser::currentUser();

        $installParameters = ['site_access_map' => ['*' => $userSiteaccess], 'top_nodes_map' => ['*' => 2], 'design_map' => ['*' => 'ezwebin_site'], 'language_map' => ['eng-GB' => $primaryLanguage->attribute( 'locale' )], 'restore_dates' => true, 'user_id' => $user->attribute( 'contentobject_id' ), 'non-interactive' => true];
        ...
        if( defined( 'EZWEBIN_INSTALLER_MAJOR_VERSION' ) && EZWEBIN_INSTALLER_MAJOR_VERSION >= "1.3" )
        {
            ...
            $webinInstaller->createSiteAccess( ['src' => ['siteaccess' => $adminSiteaccess], 'dst' => ['siteaccess' => 'ezwebin_site_admin']] );
            $webinInstaller->createSiteAccess( ['src' => ['siteaccess' => $userSiteaccess], 'dst' => ['siteaccess' => 'ezwebin_site']] );
        ...
        else
        {
            ...
            $params['user_siteaccess_list'] = array_merge( [$params['user_siteaccess']],
            ...
            $siteaccessUrls = ['admin'       => $siteInstaller->createSiteaccessUrls( ['siteaccess_list' => [$params['admin_siteaccess']], 'access_type' => $accessType, 'port' => $adminAccessTypeValue, 'host' => $params['host']] ), 'user'        => $siteInstaller->createSiteaccessUrls( ['siteaccess_list' => [$params['user_siteaccess']], 'access_type' => $accessType, 'port' => $accessTypeValue, 'host' => $params['host']] ), 'translation' => $siteInstaller->createSiteaccessUrls( [
                'siteaccess_list' => $params['language_based_siteaccess_list'],
                'access_type' => $accessType,
                'port' => $accessTypeValue + 1,
                // $accessTypeValue is for 'ezwein_site_user', so take next port number.
                'host' => $params['host'],
                'exclude_port_list' => [$adminAccessTypeValue, $accessTypeValue],
            ] )];
            ...
            $siteInstaller->createSiteAccess( ['src' => ['siteaccess' => $adminSiteaccess], 'dst' => ['siteaccess' => 'ezwebin_site_admin']] );
            $siteInstaller->createSiteAccess( ['src' => ['siteaccess' => $userSiteaccess], 'dst' => ['siteaccess' => 'ezwebin_site']] );
            ...
            $settings = [];
            $settings[] = ['settings_dir' => 'settings/siteaccess/' . $params['user_siteaccess'], 'groups' => eZSiteINISettings( $params )];
            $settings[] = ['settings_dir' => 'settings/siteaccess/' . $params['admin_siteaccess'], 'groups' => eZSiteAdminINISettings( $params )];
            $settings[] = ['settings_dir' => 'settings/override', 'groups' => eZSiteCommonINISettings( $params )];
            ...
            $siteInstaller->updateRoles( ['roles' => eZSiteRoles( $params )] );
            $siteInstaller->updatePreferences( ['prefs' => eZSitePreferences( $params )] );
            ...
            $siteInstaller->updateContentObjectAttributes( ['object_id' => $webinInstaller->setting( 'template_look_object_id' ), 'attributes_data' => $templateLookData] );


            $siteInstaller->swapNodes( ['src_node' => ['name' => "eZ Publish"], 'dst_node' => ['name' => "Home"]] );
            $siteInstaller->removeContentObject( ['name' => 'eZ Publish'] );
