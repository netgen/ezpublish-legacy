...
$conds['status'] = [[eZWorkflow::STATUS_DEFERRED_TO_CRON, eZWorkflow::STATUS_FETCH_TEMPLATE, eZWorkflow::STATUS_REDIRECT, eZWorkflow::STATUS_WAITING_PARENT]];
$db = eZDB::instance();
...
$limitList = [1 => 10, 2 => 25, 3 => 50, 4 => 100];
$limit = 10;
...
foreach ( $plist as $p )
{
    ...
    $triggers = eZTrigger::fetchList( ['module_name' => $mementoChildData['module_name'], 'function_name' => $mementoChildData['operation_name'], 'name' => $mementoChildData['name']] );
    if ( (is_countable($triggers) ? count( $triggers ) : 0) > 0 )
    {
        $trigger = $triggers[0];
        if ( is_object( $trigger ) )
        {
            ...
            {
                $outList2[ $nkey ] = ['trigger' => $trigger, 'process_list' => []];
            }
...
$Result['path'] = [['text' => ezpI18n::tr( 'kernel/workflow', 'Workflow' ), 'url' => false], ['text' => ezpI18n::tr( 'kernel/workflow', 'Process list' ), 'url' => false]];
